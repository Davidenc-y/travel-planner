...
public class ItineraryVersionPortImpl implements ItineraryVersionPort {
    ...
    private Optional<Long> refine(Itinerary current, String sessionId, String userInput,
        ...
        java.math.BigDecimal refineBudget = explicitInputParser.parseBudget(userInput);
        Integer refineDays = explicitInputParser.parseDays(userInput);
        String refineStart = explicitInputParser.extractStartDate(routePlanJson);
        // M28-9：同行人确定性解析（"同行改成情侣"→情侣；未提及 null 不覆盖）
        String refineParty = explicitInputParser.parseParty(userInput);
    ...
    private Optional<Long> createFromChat(Long userId, String sessionId,
        ...
        entity.setDays(days);
        entity.setBudget(explicitInputParser.parseBudget(userInput));
        // M28-12：首版建程解析兴趣（JSON 数组文本，与 /plan 表单路径同格式）
        List<String> interests = explicitInputParser.parseInterests(userInput);
        if (interests != null) {
            entity.setInterests(JsonUtils.toJson(interests));
        }
        // M28-9：同行人同确定性解析（"和家人去成都"→家庭；未提及 null）
        entity.setParty(explicitInputParser.parseParty(userInput));
        // M28-3：出发日期取 routePlan 首日（与 /plan 表单路径对齐，start_date 列不再恒空）
        entity.setStartDate(explicitInputParser.extractStartDate(routePlanJson));
    ...
    private void applyRefinedConstraints(Itinerary current, String userInput, String routePlanJson) {
        applyRefinedConstraints(current, explicitInputParser.parseBudget(userInput),
                explicitInputParser.parseDays(userInput),
                explicitInputParser.extractStartDate(routePlanJson), explicitInputParser.parseParty(userInput),
                explicitInputParser.parseInterests(userInput));
    }
    ...
    // R1.1：既有测试的包级签名兼容 shim（方法体已原样迁出 writeback/ExplicitInputParser，行为与文案零变更）
    static String extractExplicitInput(String composed) {
        return ExplicitInputParser.extractExplicitInput(composed);
    }

    static String parseParty(String input) {
        return ExplicitInputParser.parseParty(input);
    }

    static List<String> parseInterests(String input) {
        return ExplicitInputParser.parseInterests(input);
    }

    Integer parseDays(String input) {
        return explicitInputParser.parseDays(input);
    }

    String parseDestination(String input) {
        return explicitInputParser.parseDestination(input);
    }

    BigDecimal parseBudget(String input) {
        return explicitInputParser.parseBudget(input);
    }
