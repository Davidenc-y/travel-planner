...
public class ItineraryWritebackProperties {
    ...
    /** 目的地：规划式（规划/安排/推荐/设计…N日游）；可选消耗"修饰语+的"（如"轻松休闲的北京"），捕获组只取纯城市名（实弹 2026-09-10：7 字捕获被 cityNameMaxLen=4 拒绝致行程创建被跳过） */
    private String destPlan = "(?:规划|安排|推荐|设计)(?:一下|一个|一次)?\\s*(?:[\\u4e00-\\u9fa5]{1,8}的)?\\s*([\\u4e00-\\u9fa5]{2,10}?)"
    ...
    /** 目的地：句首式（成都3日游；含"修饰语+的"前缀消耗，与 destPlan 同口径） */
    private String destLeading = "^(?:[\\u4e00-\\u9fa5]{1,8}的)?([\\u4e00-\\u9fa5]{2,10}?)\\s*(?:的)?"
