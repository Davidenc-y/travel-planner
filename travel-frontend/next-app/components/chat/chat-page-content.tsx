...
/**
 * M28-17：聊天页全部内容（自 app/chat/page.tsx 原样迁出）。
 *
 * <p>迁移动机：IDEA/IDEA 系 IDE 对 Next.js "use client" 入口文件（page.tsx）启用
 * TS71007 检查——入口文件内组件的 props 必须可序列化，导致页面内部正常的
 * client-to-client 回调传参（onSend/onToggle/…）全部误报。组件迁至普通文件后，
 * page.tsx 仅保留无 props 薄壳，警告全部消除；行为零变化。</p>
 */
...
function ChatContent() {
  ...
  useEffect(() => {
    ...
    if (away && !away.handled) {
      ...
      if (away.preferenceSync) {
        // M28-17：mergeTags 函数式读最新（防陈闭包覆盖既有标签）
        preference.mergeTags(currentSessionId,
          (prev) => mergePreferenceSync(prev, away.preferenceSync!));
      }
  ...
  useEffect(() => {
    ...
    if (wasActive) {
      ...
      if (sid) {
        ...
        if (away && !away.handled) {
          if (away.preferenceSync) {
            // M28-17：mergeTags 函数式读最新（setTags+tagsOf 同帧双写会用陈闭包覆盖）
            preference.mergeTags(sid, (prev) => mergePreferenceSync(prev, away.preferenceSync!));
          }
          if (away.suggestion?.type === 'ANCHOR_NEW_ITINERARY') {
            setPendingSuggestion({ itineraryId: away.suggestion.itineraryId, title: away.suggestion.title });
          }
  ...
  const handleSend = async (
    ...
    try {
      ...
      // 草稿非空）；预选锚定已随后端消息携带持久化（ChatService replaceAnchors）
      if (hadNoSession) {
      ...
      // M26-F3/M28-17：本轮有效约束回写 → 偏好标签动态同步。必须用 mergeTags
      // 函数式读最新值——此前 tagsOf(sid!) 读的是发送帧的旧闭包（当时迁移刚调度、
      // sid 键尚不存在），merge({}, sync) 会把上面刚迁移的用户设置（party/interests）
      // 整个覆盖丢失（2026-09-08 实测：localStorage 只剩 destination/days/startDate）
      if (streamed.preferenceSync) {
        preference.mergeTags(sid!, (prev) => mergePreferenceSync(prev, streamed.preferenceSync!));
      }
